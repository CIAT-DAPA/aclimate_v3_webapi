...
@router.get("/by-month-range-location-ids-and-specific-measures", response_model=List[dict])
def get_climatology_by_month_range_location_ids_and_measures(
    ...
    """
    Returns  climatology data for specific location IDs, measures, and within a month range.
    ...
    for loc_id in ids:
        ...
        filtered_data = [
            d for d in data
            if start_month <= d.month <= end_month and d.measure and d.measure.short_name in measure_short_names
        ]
